...
parser.set_defaults(keep_latest=False, log=True,
                    log_gpu=False, interrupt=True, autoscale=True)
...
if args.autoscale and args.batch_size != 8:
    ...
    if __name__ == '__main__':
        print('Scaling parameters by %.2f to account for a batch size of %d.' %
...
class CustomDataParallel(nn.DataParallel):
    ...
    def gather(self, outputs, output_device):
        ...
        for k in outputs[0]:
            out[k] = torch.stack([output[k].to(output_device)
...
def train():
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                if iteration % 10 == 0:
                    ...
                    for k in loss_types:
                        if k in losses:
                            loss_dict[k] = loss_avgs[k].get_avg()
                    loss_dict['total'] = total
                    writer.add_scalars(
                        "Loss/train", loss_dict, epoch + iteration)
        ...
        compute_validation_map(epoch, iteration, yolact_net,
                               val_dataset, log if args.log else None)
...
def compute_validation_loss(net, data_loader, criterion):
    ...
    with torch.no_grad():
        ...
        loss_labels = sum([[k, losses[k]]
                           for k in loss_types if k in losses], [])
        print(('Validation ||' + (' %s: %.3f |' * len(losses)) + ')') %
              tuple(loss_labels), flush=True)
...
def compute_validation_map(epoch, iteration, yolact_net, dataset, log: Log = None):
    with torch.no_grad():
        ...
        if log is not None:
            log.log('val', val_info, elapsed=(end - start),
